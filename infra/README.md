...
The Worker has no ingress. APIM, ACS Email, Entra, Auth.js, and database values are Container Apps Key Vault references resolved through the user-assigned managed identity. Bicep creates a dedicated VNet and integrates the Container Apps environment. Key Vault public access is disabled by policy and resolves through a private endpoint and `privatelink.vaultcore.azure.net` private DNS zone. PostgreSQL resolves through a second private endpoint and `privatelink.postgres.database.azure.com`; public database access remains enabled only so the deployment workflow can temporarily allow its runner IP for migrations. The existing ACS Communication Services resource must be in the deployment resource group; Bicep grants the worker identity Contributor access scoped only to that resource.
