...
普通员工和经理不需要管理员收集 Object ID，也不需要提前创建本地用户。当前 tenant 中的用户首次通过 Entra SSO 登录时，系统会从 token 读取 Object ID、邮箱和显示名称，自动创建 ACTIVE EMPLOYEE。Guest 的 `#EXT#` UPN 会优先使用 token 的真实邮箱 claim，缺少该 claim 时再解析外部 UPN。后续登录只按不可变 Object ID 识别。

管理员在用户首次登录后，通过 **用户与角色** 页面把需要审批权限的用户提升为 MANAGER，并按评审范围配置员工的审批经理。没有审批经理的员工不会阻止周期开启，但本周期不会为其生成自评。`BOOTSTRAP_ADMIN_OBJECT_IDS` 只用于系统中尚无管理员时的首次引导。

如果历史记录绑定了错误的 Object ID，编辑该用户并选择 **重置 SSO 绑定**，再让用户本人重新登录。
