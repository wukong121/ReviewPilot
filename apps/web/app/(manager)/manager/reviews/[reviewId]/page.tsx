  ...
  return (
    <main className="page-container">
      ...
      <div className="detail-grid">
        ...
        <aside><section className="summary-panel"><h2>确定性统计</h2>{version.scores ? <><strong className="score-number">{version.scores.weightedScore.toFixed(1)}</strong><p>加权总分</p><p>能力均分 {version.scores.capabilityAverage.toFixed(1)} · 行为 {version.scores.behaviorCount}/9</p></> : <p>尚未计算</p>}</section><section className="summary-panel"><h2>AI 辅助总结</h2>{version.aiSummary ? <pre className="summary-json">{JSON.stringify(version.aiSummary, null, 2)}</pre> : <p>总结生成中或尚不可用。</p>}</section>{version.status === "PENDING_REVIEW" && <ApprovalPanel versionId={version.id} lockVersion={version.lockVersion} canDecide={review.canDecide} />}</aside>
      </div>
    </main>
  );
}
