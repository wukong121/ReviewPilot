...
import { createCycle, listCycles, openCycle } from "../../../../lib/admin/admin-service";
...
const UpdateCycleSchema = z.object({ id: z.string().uuid(), action: z.literal("OPEN") });
...
export async function PATCH(request: Request): Promise<Response> { try { const actor = await requirePermission("admin:manage-cycles"); const input = UpdateCycleSchema.parse(await request.json()); return Response.json(await openCycle(input.id, actor)); } catch (error) { return errorResponse(error); } }
