...
export function UserManager({ initialUsers }: { initialUsers: UserRow[] }) {
  ...
  function edit(user: UserRow) {
    ...
    setEntraObjectId(user.entraObjectId);
    setEmail(user.email);
  ...
  return (
    <>
      <section className="admin-panel" aria-labelledby="user-form-title">
        <div className="section-heading"><div><h2 id="user-form-title">{editingId ? "更新用户" : "添加用户"}</h2><p>Object ID 必须来自当前 Entra tenant。员工需指定一位具有经理角色的审批人。</p></div></div>
        <form className="admin-form" onSubmit={(event) => void submit(event)}>
          <label>姓名<input type="text" required value={displayName} onChange={(event) => setDisplayName(event.target.value)} /></label>
          <label>邮箱<input type="email" required value={email} onChange={(event) => setEmail(event.target.value)} /></label>
          <label>Entra Object ID<input type="text" required pattern="[0-9a-fA-F-]{36}" value={entraObjectId} onChange={(event) => setEntraObjectId(event.target.value)} /></label>
